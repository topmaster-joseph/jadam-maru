...
function workspaceBootstrap() {
  return `<script type="module" data-ekodi-workspace-handoff>const SUPABASE_URL=${JSON.stringify(supabaseUrl)};const PUBLISHABLE_KEY=${JSON.stringify(publishableKey)};const ACCESS=${JSON.stringify(accessApi)};const MARKETING_API=${JSON.stringify(marketingApi)};const ALLOWED=new Set(${JSON.stringify(allowedReturnOrigins)});const ORIGIN_TENANT=${JSON.stringify(preferredTenantByOrigin)};const STORAGE='ekodi-marketing-workspace';const hash=new URLSearchParams(location.hash.replace(/^#/,''));const tokenHash=hash.get('ekodi_token');const handedWorkspace=hash.get('ekodi_workspace');const handedTenant=hash.get('ekodi_tenant');const handedStore=hash.get('ekodi_store');const timeout=(promise,ms,label)=>Promise.race([promise,new Promise((_,reject)=>setTimeout(()=>reject(new Error(label||'timeout')),ms))]);let createClient;try{({createClient}=await timeout(import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm'),6000,'supabase_primary_timeout'))}catch(primary){console.warn('Marketing AI Supabase primary CDN failed; trying fallback',primary);try{({createClient}=await timeout(import('https://esm.sh/@supabase/supabase-js@2?bundle'),6000,'supabase_fallback_timeout'))}catch(fallback){console.error('Marketing AI Supabase client unavailable',fallback);const retry=document.querySelector('#googleCustomerAuth');if(retry){retry.textContent='로그인 연결 다시 시도';retry.title='인증 모듈 연결이 지연되고 있습니다. 다시 시도해 주세요.'}document.body.dataset.ekodiAuthError='client-unavailable';throw fallback}}const sb=createClient(SUPABASE_URL,PUBLISHABLE_KEY,{auth:{detectSessionInUrl:true,persistSession:true}});let handoffError=null;if(tokenHash){try{const{error}=await timeout(sb.auth.verifyOtp({token_hash:tokenHash,type:'email'}),10000,'verify_otp_timeout');if(error)throw error}catch(e){handoffError=e;console.error('EKODI central handoff',e)}}const dynamicAiOrigin=()=>location.protocol==='https:'&&/^[a-z0-9-]+\\.ai\\.ekodi\\.kr$/i.test(location.hostname);const auth=document.querySelector('#googleCustomerAuth');if(auth){const returnTo=(ALLOWED.has(location.origin)||dynamicAiOrigin())?location.origin+location.pathname:'https://marketing.ekodi.kr/';auth.href='https://auth.ekodi.kr/?site=marketing&return_to='+encodeURIComponent(returnTo)}const{data:{session}}=await sb.auth.getSession();if(session&&(tokenHash||handedWorkspace||handedTenant||handedStore)){history.replaceState({},document.title,location.pathname+location.search)}else if(!session&&tokenHash&&auth){auth.textContent='로그인 연결 다시 시도';auth.title=handoffError?.message||'로그인 정보를 적용하지 못했습니다. 다시 시도해 주세요.';document.body.dataset.ekodiAuthError='handoff-failed'}async function canonicalStoreId(){if(!dynamicAiOrigin())return'';try{const r=await fetch(MARKETING_API+'/api/marketing/workspace/resolve?host='+encodeURIComponent(location.hostname),{cache:'no-store'});if(!r.ok)return'';const d=await r.json();return String(d?.workspace?.storeId||'')}catch(e){console.error('EKODI canonical workspace resolve',e);return''}}async function resolveWorkspace(){if(!session)return null;try{const [r,hostStore]=await Promise.all([fetch(ACCESS+'/workspaces?site=marketing',{headers:{apikey:PUBLISHABLE_KEY,Authorization:'Bearer '+session.access_token},cache:'no-store'}),canonicalStoreId()]);if(!r.ok)return null;const d=await r.json();const list=Array.isArray(d.workspaces)?d.workspaces:[];const stored=sessionStorage.getItem(STORAGE);const requested=handedWorkspace||stored||'';let selected=requested?list.find(item=>item&&item.workspace_key===requested):null;if(requested&&!selected)sessionStorage.removeItem(STORAGE);if(!selected&&handedStore)selected=list.find(item=>item&&String(item.store_id||'')===handedStore&&['active','pre_registered'].includes(String(item.status||'')))||null;if(!selected&&handedTenant)selected=list.find(item=>item&&String(item.tenant_id||'')===handedTenant&&['active','pre_registered'].includes(String(item.status||'')))||null;if(hostStore){const hostSelected=list.find(item=>item&&String(item.store_id||'')===hostStore&&['active','pre_registered'].includes(String(item.status||'')));if(hostSelected)selected=hostSelected}const preferredTenant=ORIGIN_TENANT[location.origin]||'';if(!selected&&preferredTenant)selected=list.find(item=>item&&item.tenant===preferredTenant&&['active','pre_registered'].includes(String(item.status||'')))||null;if(!selected){selected=list.find(item=>item&&item.source==='registry'&&['active','pre_registered'].includes(String(item.status||'')))||list.find(item=>item&&item.workspace_kind==='personal')||list[0]||null}if(selected?.workspace_key)sessionStorage.setItem(STORAGE,selected.workspace_key);return selected}catch(e){console.error('EKODI workspace resolve',e);return null}}function exposeWorkspace(selected){if(!selected)return;const context=Object.freeze({workspaceKey:selected.workspace_key||null,workspaceKind:selected.workspace_kind||null,workspaceName:selected.workspace_name||null,tenantId:selected.tenant_id||null,tenant:selected.tenant||null,storeId:selected.store_id||null,store:selected.store||null,storeName:selected.store_name||null,role:selected.role||null,plan:selected.plan||'free',status:selected.status||null,source:selected.source||null});window.EKODI_MARKETING_CONTEXT=context;document.body.dataset.ekodiWorkspace=context.workspaceKey||'';document.body.dataset.ekodiWorkspaceKind=context.workspaceKind||'';document.body.dataset.ekodiTenant=context.tenantId||'';document.body.dataset.ekodiStore=context.storeId||'';window.dispatchEvent(new CustomEvent('ekodi:workspace-ready',{detail:context}))}if(session){const selected=await resolveWorkspace();exposeWorkspace(selected);document.body.dataset.ekodiAuthError='';if(auth){const plan=String(selected?.plan||'free').toUpperCase();const name=selected?.workspace_name||selected?.store_name||(selected?.workspace_kind==='personal'?'개인':'Marketing AI');auth.textContent=name+' · '+plan+' ✓';auth.title=(session.user.email||'EKODI 회원')+' · '+name+' · '+plan}}}</script>`;
}
...
console.log(`Marketing AI selected workspace handoff + resilient Supabase CDN fallback patched: ${patched} pages`);
