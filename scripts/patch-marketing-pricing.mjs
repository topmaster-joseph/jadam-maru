...
function planMarkup() {
  return (pricing.plans || []).map(plan => {
    ...
    return `<article class="plan-card" data-plan="${esc(plan.id)}"><span class="plan-ribbon">추천</span><div class="plan-name">${esc(plan.label)}</div><div class="plan-price">${esc(price)} <span>${plan.id === 'free' ? '체험' : '기본료'}</span></div><p class="plan-summary">${esc(plan.summary)}</p><ul class="plan-benefits">${benefits}</ul><div class="plan-estimate"><small>선택한 사용량 예상</small><strong data-plan-total="${esc(plan.id)}">${plan.id === 'free' ? '무료 체험' : '-'}</strong></div><a class="pricing-cta" href="${authHref}">${plan.id === 'free' ? '무료로 시작' : '로그인 후 선택'}</a></article>`;
  }).join('');
}
...
for (const required of ['id="pricing"', 'id="usageEstimator"', 'data-plan="free"', 'data-plan="flex"', 'data-plan="plus"', 'data-plan="pro"', 'data-plan="auto"', 'BEST PLAN AI', '1회 결제', '종량제', '자동화 플랜']) {
