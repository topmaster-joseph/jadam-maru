...
// Public hub must be discoverable and must not promise an anonymous trial when
// the actual free workspace is opened after the central Google login.
html = html
...
/* Anonymous landing: one promise, one preview, one primary action. */
body:not(.member-session-mode) main{width:min(980px,100%);padding:18px 20px 36px}
...
body:not(.member-session-mode) .member-preview{max-width:800px;margin:18px auto 0;padding:0;border:0;background:transparent;box-shadow:none}
...
/* These repeat the same promise and CTA on the anonymous page. Keep them for
   authenticated workspace/pricing flows, but remove the public-page echo. */
...
/* Pro domain management belongs to an authenticated workspace, never to the
   public landing even if session restoration finishes before workspace mode. */
...
for (const required of [
  ...
  'body:not(.member-session-mode) .member-preview{max-width:800px',
  'body:not(.member-session-mode) .workflow-v2,',
  'body:not(.member-session-mode) .custom-domain-panel{display:none!important}',
  'FREE START · 먼저 체험',
]) {
