...
for(const required of ['id="memberPreview"','id="memberTrial"','id="freeTrialForm"','Google로 무료 체험 시작','data-free-member-experience','free-member-mode','dashboardPreview']){
  if(!html.includes(required))throw new Error(`Free-member experience contract missing: ${required}`);
}

fs.writeFileSync(hubFile,html);
console.log('✅ Marketing AI landing is explanation-first; FREE members enter a focused hands-on workspace without the dashboard preview');
