...
const runtime=`<script data-free-member-experience>(()=>{const KEY='ekodi-marketing-free-experience';const url=new URL(location.href);const auth=document.querySelector('#googleCustomerAuth');const preview=document.querySelector('#memberPreview');const trial=document.querySelector('#memberTrial');const pricing=document.querySelector('#pricing');const pricingIntro=document.querySelector('#memberPricingIntro');const headerFree=document.querySelector('.header-link[href="#memberPreview"]');const welcomed=url.searchParams.get('welcome')==='free';if(welcomed)localStorage.setItem(KEY,'1');const hasMemberSignal=()=>localStorage.getItem(KEY)==='1'||welcomed||/✓|이용중/.test(auth?.textContent||'');function render(){const member=hasMemberSignal();if(member)localStorage.setItem(KEY,'1');if(preview)preview.hidden=member;if(trial)trial.hidden=!member;if(pricing)pricing.hidden=!member;if(pricingIntro)pricingIntro.hidden=!member;if(headerFree){headerFree.href=member?'#memberTrial':'#memberPreview';headerFree.textContent=member?'무료 체험':'무료 체험';}if(member&&auth&&!/✓|이용중/.test(auth.textContent||'')){auth.textContent='무료 체험 중 ✓';auth.href='#memberTrial';}if(member&&(welcomed||location.hash==='#memberTrial'))setTimeout(()=>trial?.scrollIntoView({behavior:'smooth',block:'start'}),120);}render();if(auth)new MutationObserver(render).observe(auth,{childList:true,subtree:true,characterData:true});if(welcomed){url.searchParams.delete('welcome');history.replaceState({},document.title,url.pathname+(url.searchParams.toString()?'?'+url.searchParams.toString():'')+'#memberTrial');}})();</script>`;

html=html.replace('</head>',`${style}</head>`);
const trialMarker='<section class="trial-lab member-trial" id="memberTrial" hidden>';
...
for(const required of ['id="memberPreview"','id="memberTrial"','id="freeTrialForm"','Google로 무료 체험 시작','data-free-member-experience']){
  if(!html.includes(required))throw new Error(`Free-member experience contract missing: ${required}`);
}

fs.writeFileSync(hubFile,html);
console.log('✅ Marketing AI landing is explanation-first; free members enter hands-on trial directly');
